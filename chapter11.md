...
論理変数は、代入ではなく *単一化 (unification)* によって代入されます。
一度代入された論理変数は変更できません。
...
たとえば、クエリ (`likes Sandy ?who`) は `?who` が `Lee or Kim` に代入されている場合に成功し、クエリ (`likes ?who Lee`) は `?who` が `Sandy` に代入されている場合に成功します。
...
単一化フレームワーク内では、変数 (上記の `?x` や `?y` など) は *論理変数* と呼ばれます。通常の変数と同様に、論理変数には値を割り当てることも、代入を解除することもできます。
違いは、論理変数は変更できないことです。
一度値が割り当てられると、その値が保持されます。異なる値と単一化しようとすると失敗します。`(?x + ?x)` と `(2 + 2)`のパターン マッチが可能であったのと同様に、同じ値を持つ変数を複数回単一化することも可能です。

単純なパターン マッチングと単一化の違いは、単一化では 2 つの変数を互いに一致させることができる点です。2 つの変数は代入されていないままですが、同等になります。どちらかの変数がその後値に代入されると、両方の変数がその値を採用します。
次の例では、`?x` を `?y` に代入して、変数 `?x` と `?y` を等しくします。
...
私たちが定義する `unify` のバージョンは、`?y` を `0` に、`?x` を `?y` に、`?a` を `?x` に代入 (bind) することでこの結果を示します。
...
(defun get-binding (var bindings)
 "代入リストから変数と値のペア'(variable . value)'を探す"
 (assoc var bindings))
(defun binding-val (binding)
 "ある代入から値部分を取り出す"
 (cdr binding))
(defun lookup (var bindings)
 "代入リストから値部分(for var)を取り出す"
 (binding-val (get-binding var bindings)))
(defun extend-bindings (var val bindings)
 "代入リストに1つの(var . value)を追加する"
 (cons (cons var val)
       ;; 1回でも「本物」の代入を追加した場合は
...
(defun match-variable (var input bindings)
 "VARが入力とマッチしているかどうか?使ってみて（もしくは更新してみて）代入を返す"
...
ここでの問題は、`?y` が1回でもそれ自体に代入されると、`unify-variable` 内の `unify` の呼び出しが無限ループを引き起こすことです。しかし、`?y` とそれ自身とのマッチングは常に成功する必要があるため、`unify` 内の等価性テストを変数テストの前に移動します。
...
同じ代入リストで `(?Y . ?X)` と `(?X . ?Y)` の両方を許可することは、`(?Y . ?Y)` を許可するのと同じくらい悪いことです。
この問題を回避するには、代入された変数ではなく、代入 リストで指定された値を処理するポリシーにする必要があります。
関数 `unify-variable` はこのポリシーを実装できません。
var が代入された変数である場合に var の代入を取得するチェックがありますが、`x` が代入された変数である場合に `x` の値を取得するチェックも必要です。
...
ここで `((?XF ?X))` は実際には `((?X . ((F ?X))))` を意味するため、 `?X` は (`F ?X`) に代入されます。
...
特に、必要なのは、代入 リストを式に置き換える関数です。
当初、関数 `sublis` が利用可能であったため、代入の実装として連想リストを選択しました。
皮肉なことに、変数は他の変数に代入され、その変数は式に代入されるため、`sublis` は機能しなくなります。
`function subst-bindings` は、再帰代入を置き換える点を除いて、`sublis` のように動作します。
...
最後に、関数 `unifier` は `unify` を呼び出し、結果の代入 リストを引数の 1 つに代入します。
`x` の選択は任意です。代入 リストを `y` に代入すると、同じ結果が得られます。
...
| `show-prolog-vars` | 各変数とその代入を出力します。 |
| `variables-in` | 式内のすべての変数のリストを返します。 |
| | **以前に定義された定数** |
| `fail` | 単一化が失敗したことを示します。 |
| `no-bindings` | 変数なしでの単一化の成功 |
| | **以前に定義された関数** |
| `unify` | 2 つの式を単位うする代入を返します (セクション 11.2)。 |
| `unify-variable` | 式に対して変数を単一化します。 |
| `occurs-check` | 式の中に特定の変数が出現するかどうかを確認します。 |
| `subst-bindings` | 代入を式に置き換えます。 |
| `get-binding` | 変数の `(var . val)` 代入を取得します。 |
| `lookup` | 変数の値を取得します。 |
| `extend-bindings` | 代入 リストに新しい変数/値のペアを追加します。 |
...
規則の場合、bodyの目標は、前のステップからの代入が維持されていることを確認しながら、一度に 1 つずつ証明する必要があります。
実装は簡単です:
...
そうしないと、証明の過程で 2 つの異なる節で使用される変数は、各節で同じ値を取る必要があり、これは間違いを引き起こします。
...
Robin が答えであることを確認するには、代入を解く必要があります。`?who` は `?x2856` に代入されており、`?x2856` は Robin に代入されています。
...
すべてのプリミティブは、プリミティブ関係への引数のリスト (ここでは表示する変数のリスト)、これらの引数の代入 リスト、および保留中の目標のリストという 3 つの引数の関数である必要があります。
...
最初のクエリでは、length は 1 つの可能な解法、つまり 2 つの代入されていない要素を含むリストのみを生成します。
...
[セクション 11.2](#s0015) で見たように、変数の代入 リストを追跡するのは少し難しいです。
また、代入 リストが大きくなると、リストを線形に検索する必要があり、代入 リストを保持するためのスペースを割り当てる必要があるため、非効率になりがちです。
別の実装としては、`unify` を破壊的な操作に変更する方法があります。
このアプローチでは、代入 リストはありません。
代わりに、各変数は、その代入用のフィールドを含む構造体として表されます。
変数が別の式と単一化されると、変数の代入 フィールドは式を指すように変更されます。
...
マクロ `deref` は変数の代入を取得し、それが代入されていない変数または非変数式である場合はその引数を返します。
変数は別の変数に代入され、その変数は最終値に代入されるため、ループが含まれます。
...
これは、成功した場合は true を返し、失敗した場合は false を返す述語であり、変数代入を変更する副作用があります。
...
変数には名前と代入の 2 つのスロットしかないことに注意してください。
代入は重要ですが、名前は結果の表示にのみ必要であり、ほとんどの変数では任意です。
これは代替実装を示唆しています。
各変数は、変数の代入のcons cellと、型を示す任意のマーカーになります。
...
**演習 11.8 [h]** 代入 リストの代わりに破壊的単一化を使用する Prolog インタープリタのバージョンを記述します。
...
別の方法としては、`unify` を変更して、引数ごとに 1 つずつ、合計 2 つの代入 リストを取得し、それらを別々に保持するようにします。
この代替案を実装してください。
