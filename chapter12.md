...
論理プログラミングの背後にある考え方は、プログラマーが問題とその解法を説明する関係を述べるべきであるというものです。
これらの関係は、問題を解決できるアルゴリズムに対する制約として機能しますが、アルゴリズムの詳細については、プログラマーではなくシステム自体が責任を負います。
「プログラミング」と「ロジック」の間の緊張については、[第 14 章](chapter14.md) で説明しますが、現時点では、Prolog は論理プログラミングの理想的な目標に近似していると言っても過言ではありません。
Prolog は、従来のプログラミング言語と論理仕様言語の間の実用上快適な場所へと到達しました。
それは 3 つの重要なアイデアに基づいています。

* Prolog では、ただ１つの *統一されたデータベース* の使用が推奨されています。
優れたコンパイラは、このデータベースへの効率的なアクセスを提供し、Lisp プログラマが詳細に処理しなければならないベクトル、ハッシュ テーブル、プロパティ リスト、およびその他のデータ構造の必要性を軽減します。
Prolog はデータベースの考え方に基づいているため、Lisp (あるいはほとんどの言語) が *関数型* であるのに対して、Prologは*リレーショナル*です。Prolog では、「サンフランシスコの人口は750,000人である」などの事実を関係として表現します。
Lisp では、都市を入力として受け取り、数値を返す関数「population」として記述する傾向があります。
関係はより柔軟であり、サンフランシスコの人口を見つけるだけでなく、たとえば人口が500,000 人を超える都市を見つけるためにも使用できます。

* Prolog は、「通常の」変数の代わりに *論理変数* を提供します。
論理変数は、代入ではなく *単一化* によって束縛されます。
一度束縛されたロジック変数は変更できません。
したがって、それらは数学の変数に似ています。
論理変数と単一化の存在​​により、論理プログラマーは、評価の順序を記述する必要なく (代入文の場合のように)、問題を制約する方程式を記述できます (数学の場合のように)。

* Prolog は *自動バックトラッキング* を提供します。
Lisp では、各関数呼び出しは単一の値を返します (プログラマが複数の値または値のリストを返すように特別に手配しない限り)。
Prolog では、各クエリは、クエリを満たすデータベース内の関係の検索につながります。複数ある場合は、それらを一度に検討されます。
「人口が 500,000 人を超え、州都となっている都市はどこか」のように、クエリが複数の関係を必要とする場合、Prolog は人口関係を調べて、人口が 500,000 人を超える都市を検索します。
見つかった都市ごとに、`首都`関係をチェックして、その都市が首都であるかどうかを確認します。もし、そうであるならProlog は都市を出力し、そうでない場合は、*バックトラック*して、`人口`関係で別の都市を見つけようとします。
したがって、Prolog を使用すると、プログラマーはデータの保存方法と検索方法の両方について心配する必要がなくなります。
いくつかの問題では、単純な自動検索は非効率すぎるため、プログラマーは問題を再定義する必要があります。
しかし、理想的なのは、Prolog プログラムが、解法がどのように達成されるかを詳細に説明せずに、解法に対する制約を述べることです。

この章の目的は 2 つあります。Lisp ではなく Prolog で特定のプログラムを記述できる可能性があることを読者に知らせること、そして 3 つの重要な Prolog のアイデアの実装を紹介して、Lisp プログラム内で (独立してまたは組み合わせて) 使用できるようにすることです。
Prolog は、プログラミングのやり方を興味深く、異なる視点から見る方法を表しており、故に知っておく価値があるのです。
以降の章では、Prolog アプローチのいくつかの便利な応用例を見ていきます。

## 11.1 アイデア 1: 統一されたデータベース

Prolog の最初の重要なアイデアは、この本の読者にはおなじみのはずです。それは、アサーションの格納されたデータベースを操作することです。
Prolog では、アサーションは *節* と呼ばれ、いくつかのオブジェクト間の関係を述べる *事実* と、偶発的な事実を述べるために使用される *ルール* の 2 つのタイプに分けられます。
...
これらの事実は、キムはロビンが好き、サンディはリーとキムの両方が好き、ロビンは猫が好き、という意味に解釈できます。
...
Prolog データベースの 2 番目のタイプの節は *ルール* です。ルールは条件付きの事実を述べます。たとえば、サンディは猫が好きな人を好きになるというルールは次のように表すことができます。
...
([第 1 章](chapter1.md) でこれを行いました。ここでは、文法規則を使用して単語の文字列と解析ツリーの両方を生成しました。)
上記のルールは、手続き的には「`x` が猫を好きだとわかったら、Sandy は `x` を好きだと結論付ける」と解釈できます。
...
たとえば、次の節は、キムはリーとキムの両方を好きな人を好きだと述べています。
...
また、2 つの構造(structure)を単一化した結果の構造を示す関数 `unifier` も定義します。
...
この禁止は、変数をその変数を含む構造と単一化しようとするたびに失敗するようにunifierを変更することによって実現できます。
...
最初の実装の選択肢は、ルールと事実の表現です。
ルールと事実を区別せずに、唯一の単一化された節データベース(a single uniform data base of caluses)を構築します。
...
データを出し入れする方法がなければ、データベースは役に立ちません。関数 prove は、指定された目標がデータベース内に直接存在する事実と一致するか、ルールから導き出せるかを証明するために使用されます。目標を証明するには、まずその目標の候補となる節をすべて見つけます。各候補について、目標が節のheadと単一化されているかどうかを確認します。真であるならば、節のbodyにあるすべての目的を証明するようにします。
実際には、bodyに目標はないので、成功はすぐに起こります。
ルールの場合、bodyの目標は、前のステップからの束縛が維持されていることを確認しながら、一度に 1 つずつ証明する必要があります。
実装は簡単です:
...
次の 3 つは、「サンディは猫が好きな人なら誰でも好きだ」という節から生じます。
まず、ロビンは猫が好きであるという事実から、ロビンが答えです。
Robin が答えであることを確認するには、束縛を解く必要があります。`?who` は `?x2856` に束縛されており、`?x2856` は Robin に束縛されています。

ここで驚きの事実が明らかになります。サンディがリストに載っているのは、次の理由からです。
...
サンディが 2 回登場するのは、サンディがサンディを好きだということを示す方法が 2 つあるためです。
...
3番目は事実の確認を求めます。
答えは「いいえ」です。ロビンがリーを好きだということを示す節や事実は存在しないからです。
...
このセクションでは、漸進的 Prolog インタープリターを実装します。
...
関数 `?` には引数がなく、`var` 構造のスロットに指定されたデフォルト値を割り当てるだけです。
...
関係節は、解釈されるルールと事実を構成する節を格納するために使用されます。
...
ただし、ロジック変数は変更できないため、特定のデータ構造と操作は提供されません。たとえば、Prolog ではベクトルの要素を更新する方法はありません。
...
ほとんどの実装では、構造は小さなオブジェクトには最適な選択ではありません。
...
どの関係がプリミティブ (Prolog データベースに格納されている) で、どの関係がルールによって導出されるかを決定する必要があります。
